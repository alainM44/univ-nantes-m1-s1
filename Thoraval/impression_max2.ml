...
(* reception du max de gauche *)
                                               (* garde sur reception message *)
    method quand_reception_max_gauche_plus_grand msg =
    ...
    method quand_reception_max_gauche_plus_petit msg =
                     (* _ pour msg qui est de toutes facons Max_gauche(_) *)
      match msg with Max_gauche(le_max_gauche) -> 
	ma_valeur <= le_max_gauche
            
                         (* commande gardee - tableau transitions - trans_all *)
    method fais_suivre_ma_valeur _ (* pour expediteur *) _ =
      self#log (" -- valeur2 : " ^ string_of_int(ma_valeur)) ; (* pour tracer *)
      self#write_all reseau_vers_droite (Max_gauche(ma_valeur))

    method fais_suivre_sa_valeur _ (* pour expediteur *) msg =
      self#log (" -- valeur1 : " ^ string_of_int(ma_valeur)) ; (* pour tracer *)
      match msg with Max_gauche(le_max_gauche) -> 
	self#write_all reseau_vers_droite (Max_gauche(le_max_gauche))
  ...
  initializer
    ...
    (** definition des transitions (une seule ici) **)
        self#trans_all En_attente_max Max_envoye 
                       reseau_vers_droite  
                       self#quand_reception_max_gauche_plus_petit
                       self#fais_suivre_sa_valeur
  initializer
        self#trans_all En_attente_max Max_envoye 
                       reseau_vers_droite  
                       self#quand_reception_max_gauche_plus_grand
                       self#fais_suivre_ma_valeur
