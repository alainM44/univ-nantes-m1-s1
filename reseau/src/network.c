...
void frag_and_send(int sock, char file_to_frag[])
{
  ...
  for (long long int  i=(long long int)1;i<=nb_f;i++)
    {
...
int fsize(const char * fname, long long int * ptr)
{
    ...
    if (f != NULL)
    {
        fseeko(f, 0, SEEK_END); /* aller a la fin du fichier */
        *ptr =ftello(f); /* lire l'offset de la position courante par rapport au debut du fichier */
        fclose(f);
    }
    else{
