...
public abstract class AbstractGrapheOriente implements GrapheOriente
{
	...
	public int NombreArcs()
	{
		...
		{
			int temps = 0;
			ArrayList<Integer> listeNoeud = listeNoeuds();
		...
		{
			ArrayList<Integer> voisins = listerSuccesseurs(n);
			d.put(temps, n);
			temps++;
			couleur.put(n, 1);
		...
		public HashMap<Integer, Integer> BFS(int n, boolean option, HashMap<Integer, Integer> horloge)
		{
			...
			HashMap<Integer, Integer> couleur = new HashMap<Integer, Integer>();
			HashMap<Integer, Integer> pi = new HashMap<Integer, Integer>();
			ArrayBlockingQueue<Integer> file = new ArrayBlockingQueue<Integer>(NombreNoeuds());
			...
			while (!file.isEmpty())
			{
				...
				temps++;
				for (Integer voisin : listerSuccesseurs(noeudCourant))
				{
					if (couleur.get(voisin) == 0)
					{
		...
		public boolean acyclicite()
		{
			...
			ArrayList<Arc> listeArcs = listeArcs();
		...
		public boolean connexite()
		{
			...
			ArrayList<Integer> listeNoeuds = listeNoeuds();
			int nbNoeuds = NombreNoeuds();
